...
public class ApigatewayApplication {
	...
	RouteLocator gatewayArtifactServiceRouter(RouteLocatorBuilder builder){
        return builder.routes()
				...
				.route(r -> r.path("/product/**").and().method("GET")
                        .filters(f-> f.rewritePath("/product","/product"))
                        .uri("http://localhost:8093"))
                .route(r -> r.path("/product").and().method("POST")
                        .filters(f-> f.rewritePath("/product","/product"))
                        .uri("http://localhost:8093"))
                .route(r -> r.path("/product/**").and().method("PUT")
                        .filters(f-> f.rewritePath("/product","/product"))
                        .uri("http://localhost:8093"))
                .route(r -> r.path("/product/**").and().method("DELETE")
                        .filters(f-> f.rewritePath("/product","/product"))
                        .uri("http://localhost:8093"))
